...
static int MAX9286_DEFAULT_MODE;
static int MAX9286_DEFAULT_WIDTH;
static int MAX9286_DEFAULT_HEIGHT;

//#define MAX9286_DEFAULT_WIDTH		5120
...
static inline int max9286_write_reg(struct i2c_client *client,u8 reg, u8 val)
{
	...
	ret = i2c_transfer(client->adapter, &msg, 1);
	if (ret < 0) {
		printk(&client->dev, "Failed writing register 0x%04x!\n", reg);
		return ret;
	}
...
static int max9286_read_reg(struct i2c_client *client, u8 reg, u8 *val)
{
	int ret;
	//u8 data[2];
...
//if g_sensor_num ==4
	MAX9286_DEFAULT_MODE = MAX9286_MODE_5120X720;
	MAX9286_DEFAULT_WIDTH = 5120;
	MAX9286_DEFAULT_HEIGHT = 720;

	// Set link order in MIPI CSI-2 output
	reg = 0xE4;  //Default setting
	if (g_sensor_num == 1) {			//只连接1个摄像头
		MAX9286_DEFAULT_MODE = MAX9286_MODE_1280X720;
		MAX9286_DEFAULT_WIDTH = 1280;
		MAX9286_DEFAULT_HEIGHT = 720;
		...
		MAX9286_DEFAULT_MODE = MAX9286_MODE_2560X720;
		MAX9286_DEFAULT_WIDTH = 2560;
		MAX9286_DEFAULT_HEIGHT = 720;
		switch (g_sensor_is_there) {
			case 0xC:
				reg = 0x4E;
		...
		MAX9286_DEFAULT_MODE = MAX9286_MODE_3840X720;
		MAX9286_DEFAULT_WIDTH = 3840;
		MAX9286_DEFAULT_HEIGHT = 720;
		switch (g_sensor_is_there) {
			case 0xE:
				reg = 0x93;
	...
	//max9271_write_reg(client, 1, 0x04, 0x83);
	//max9271_write_reg(client, 2, 0x04, 0x83);
	//max9271_write_reg(client, 3, 0x04, 0x83);
	//max9271_write_reg(client, 4, 0x04, 0x83);
	mdelay(100);  //Wait for more than 2 frame time
...
#define OV490_COLOR_HUE_MIN		0
#define OV490_COLOR_HUE_MAX		23
#define OV490_COLOR_HUE_DEFAULT		12
#define OV490_COLOR_SATURATION_MIN	0
#define OV490_COLOR_SATURATION_MAX	7
#define OV490_COLOR_SATURATION_DEFAULT	2
#define OV490_BRIGHITNESS_MIN		0
#define OV490_BRIGHITNESS_MAX		4
#define OV490_BRIGHITNESS_DEFAULT
#define OV490_SHARPNESS_MIN		0
#define OV490_SHARPNESS_MAX		10
#define OV490_SHARPNESS_DEFAULT		0
#define OV490_CONTRAST_MIN		0
#define OV490_CONTRAST_MAX		10
#define OV490_CONTRAST_DEFAULT		0
#define OV490_EXPOSURE_MIN
#define OV490_EXPOSURE_MAX
#define OV490_EXPOSURE_DEFAULT
#define OV490_AWB_RGB_MIN
#define OV490_AWB_RGB_MAX
#define OV490_AWB_RGB_DEFAULT
#define OV490_FRAME_RATE_MIN		12
#define OV490_FRAME_RATE_MAX		30
#define OV490_FRAME_RATE_DEFAULT	30
#define OV490_
#define OV490_
#define OV490_
#define OV490_
...
#define MAX9286_MAX_GAIN 			1000
#define MAX9286_DEFAULT_GAIN 			2
...
#define MAX9286_MAX_EXPOSURE_COARSE		200
#define MAX9286_DEFAULT_EXPOSURE_COARSE		3
...
static struct v4l2_ctrl_config ctrl_config_list[] = {
...

	{
		...
		.id = V4L2_CID_FRAME_RATE,		//frame rate
		.name = "Frame_rate",
		.type = V4L2_CTRL_TYPE_INTEGER,
		.flags = V4L2_CTRL_FLAG_SLIDER,
		.min = OV490_FRAME_RATE_MIN,
		.max = OV490_FRAME_RATE_MAX,
		.def = OV490_FRAME_RATE_DEFAULT,
		.step = 1,
	},

	{
		.ops = &max9286_ctrl_ops,
		.id = V4L2_CID_SHARPNESS,		//Sharpness
		.name = "Sharpness",
		.type = V4L2_CTRL_TYPE_INTEGER,
		.flags = V4L2_CTRL_FLAG_SLIDER,
		.min = OV490_SHARPNESS_MIN,
		.max = OV490_SHARPNESS_MAX,
		.def = OV490_SHARPNESS_DEFAULT,
		.step = 1,
	},

	{
		.ops = &max9286_ctrl_ops,
		.id = V4L2_CID_CONTRAST,		//contrast
		.name = "Color contrast",
		.type = V4L2_CTRL_TYPE_INTEGER,
		.flags = V4L2_CTRL_FLAG_SLIDER,
		.min = OV490_CONTRAST_MIN,
		.max = OV490_CONTRAST_MAX,
		.def = OV490_CONTRAST_DEFAULT,
		.step = 1,
	},

	{
		.ops = &max9286_ctrl_ops,
		.id = V4L2_CID_SATURATION,		//saturation
		.name = "Color saturation",
		.type = V4L2_CTRL_TYPE_INTEGER,
		.flags = V4L2_CTRL_FLAG_SLIDER,
		.min = OV490_COLOR_SATURATION_MIN,
		.max = OV490_COLOR_SATURATION_MAX,
		.def = OV490_COLOR_SATURATION_DEFAULT,
		.step = 1,
	},

	{
		...
		.type = V4L2_CTRL_TYPE_INTEGER,
		.flags = V4L2_CTRL_FLAG_SLIDER,
		.min = OV490_COLOR_HUE_MIN,
		.max = OV490_COLOR_HUE_MAX,
		.def = OV490_COLOR_HUE_DEFAULT,
		.step = 1,
	},
...
{	
	...
	control.id = V4L2_CID_FRAME_RATE;
	err = v4l2_g_ctrl(&priv->ctrl_handler, &control);	
	if (err)
		dev_dbg(&client->dev, "%s: warning gain override failed\n",__func__);


	//unsigned char param[13]= {0x01,0x00,0x80,0x00,0x50,0x00,0x2A,0x01,0x00,0x01,0x00,0x01,0x00};
...
/*****************************************************************************
	...
	host_control_set(client, 1, CMD_FRAMERATE_SET, &state, 1);
	host_control_set(client, 2, CMD_FRAMERATE_SET, &state, 1);

	printk("frame rate is %d\n", control.value); 
	...
	host_control_set(client, 1, CMD_SHARPNESS_SET, &state, 1);
	host_control_set(client, 2, CMD_SHARPNESS_SET, &state, 1);

	printk("sharpness is %d\n", control.value); 
	...
	host_control_set(client, 1, CMD_CONTRAST_SET, &state, 1);
	host_control_set(client, 2, CMD_CONTRAST_SET, &state, 1);

	printk("color contrast is %d\n", control.value); 
	...
	host_control_set(client, 1, CMD_SATURATION_SET, &state, 1);
	host_control_set(client, 2, CMD_SATURATION_SET, &state, 1);

	printk("color saturation is %d\n", control.value); 
	...
	err = v4l2_g_ctrl(&priv->ctrl_handler, &control);	
	if (err)
		dev_dbg(&client->dev,"%s: warning frame length override failed\n",__func__);

	control.id = V4L2_CID_COARSE_TIME;
	err = v4l2_g_ctrl(&priv->ctrl_handler, &control);
	
	if (err)
		dev_dbg(&client->dev,"%s: warning coarse time override failed\n",__func__);

	control.id = V4L2_CID_COARSE_TIME_SHORT;
	err = v4l2_g_ctrl(&priv->ctrl_handler, &control);	
	if (err)
		dev_dbg(&client->dev,"%s: warning coarse time short override failed\n",	__func__);
...
static int max9286_ctrls_init(struct max9286_priv *priv, bool eeprom_ctrl)
{
	...
	printk("numctrls = ARRAY_SIZE(ctrl_config_list); numctrls=%d\n", numctrls);
...
error:
	printk("err when max9286_ctrls_init()\n");
...
static int max9286_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("MAX9286_DEFAULT_MODE = %d\n", MAX9286_DEFAULT_MODE);
	printk("MAX9286_DEFAULT_WIDTH = %d\n", MAX9286_DEFAULT_WIDTH);
	printk("MAX9286_DEFAULT_HEIGHT = %d\n", MAX9286_DEFAULT_HEIGHT);
	...
	printk("11common_data->def_width = %d\n", common_data->def_width);
	printk("11common_data->def_height = %d\n", common_data->def_height);
